...
# AUTO: Изменяем timeout в hivemind
echo_green ">> Применяем автоматические настройки..."
echo_green ">> Изменяем startup_timeout в HIVEMind на 120 секунд..."
sed -i 's/startup_timeout: float = 15/startup_timeout: float = 120/g' "$(python3 -c 'import hivemind.p2p.p2p_daemon as m; print(m.__file__)' 2>/dev/null || echo '/dev/null')" 2>/dev/null || echo "Hivemind пока не установлен, timeout будет изменен позже"
...
# AUTO: Устанавливаем значения без вопросов
HF_TOKEN=${HF_TOKEN:-""}
HUGGINGFACE_ACCESS_TOKEN="None"
echo_green ">> Автоматически устанавливаем HUGGINGFACE_ACCESS_TOKEN=None (без загрузки в HuggingFace Hub)"

MODEL_NAME=""
echo_green ">> Используем модель по умолчанию из конфига"

echo_green ">> Good luck in the swarm!"
echo_blue ">> And remember to star the repo on GitHub! --> https://github.com/gensyn-ai/rl-swarm"

# AUTO: Применяем изменения после установки пакетов
